...
export default function Posts(props) {
  ...
  const fetchInstaPosts = async () => {
    ...
    await axios
      ...
      // .get(`${FEED_THUMBNAILS}/${props.match.params.id}`)
      // .get(`${FEED_THUMBNAILS}/${5}`)
      .then((response) => {
  ...
  }, [postPaging]); // 이부분 추후 수정(다음 데이터 필요할때)
  ...
  const handleScroll = () => {
    ...
    if (scrollTop + clientHeight >= scrollHeight) {
      console.log("스크롤 계산 내부");
      // const nextPostPaging = postPaging + 9;
      setPostPaging((prev) => prev + 9);
    }
  };
